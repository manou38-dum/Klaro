...
export default function ResultCard({ result }: any) {
  ...
  if (!result) return <div className="text-center py-12">Chargement...</div>;
  if (result.error) return <div className="text-center py-12 text-red-600">Erreur: {result.error}</div>;
      ...
      <div className={`max-w-md mx-auto ${isVisible ? "opacity-100" : "opacity-0"} transition-opacity`}>
        <div className={`rounded-3xl shadow-2xl overflow-hidden bg-white ring-4 ${modeConfig.ring}`}>
          <div className={`bg-gradient-to-br ${modeConfig.gradient} p-6 text-white text-center`}>
            ...
            <p className="text-lg">{result.insight_principal}</p>
          </div>
            ...
            {motsCles.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {motsCles.map((mot: string, i: number) => (
                  <span key={i} className="px-3 py-1 bg-violet-100 text-violet-700 rounded-full text-sm">#{mot}</span>
                ))}
              </div>
            )}
            {result.ressenti_global && <p className="italic text-slate-600">{result.ressenti_global}</p>}
            {result.conseil_rapide && <p className="text-slate-700"><strong>Conseil:</strong> {result.conseil_rapide}</p>}
          </div>
          ...
          {traits.length > 0 && traits.map((trait: any, i: number) => (
            <div key={i} className="rounded-xl p-4 bg-white border-2 border-slate-200">
              <div className="flex justify-between items-start mb-2">
                <h4 className="font-bold text-slate-900">{trait.trait || "Trait"}</h4>
                <span className="text-xs px-2 py-1 bg-slate-100 rounded">{trait.score_label || "Moyen"}</span>
              </div>
              {trait.score_polarise !== undefined && <BigFiveGauge score={trait.score_polarise} dimension={trait.bigfive_dimension || "N"} />}
              <p className="text-sm text-slate-600 mt-2 italic">{trait.analyse || ""}</p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white p-6">
          ...
          <div className="p-4 bg-slate-50 rounded-lg">
            <p className="text-sm text-slate-700">
              {activeTab === "autorite" ? rapports.autorite : activeTab === "pairs" ? rapports.pairs : rapports.action}
            </p>
          </div>
        </div>
          ...
          <div className="p-6 bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-base">{result.conseil}</p>
              </div>
            </div>
          </div>
        )}
