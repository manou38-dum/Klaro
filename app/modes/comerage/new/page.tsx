...
const INTENSITIES = [
  { 
    ...
    label: "Light", 
    emoji: "🌸", 
    activeColor: "bg-emerald-500 text-white border-emerald-600 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]" 
  },
  { 
    id: "epice", 
    label: "Épicé", 
    emoji: "🌶️", 
    activeColor: "bg-amber-500 text-white border-amber-600 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]" 
  },
  { 
    id: "caliente", 
    label: "Caliente", 
    emoji: "🔥", 
    activeColor: "bg-orange-500 text-white border-orange-600 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]" 
  },
  { 
    id: "defouloir", 
    label: "Défouloir", 
    emoji: "💥", 
    activeColor: "bg-rose-600 text-white border-rose-700 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]" 
  }
];
...
export default function NewComerageScene() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    sessionStorage.setItem("pendingIntensity", intensity);
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-rose-50 to-amber-50 pb-20">
      <div className="max-w-md mx-auto p-4">
        ...
        <div className="bg-gradient-to-br from-rose-500 to-orange-500 rounded-2xl p-5 text-white mb-6 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          ...
          <p className="text-sm font-medium opacity-90">Choisis le ton, puis raconte-moi la situation...</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          
          {/* Sélecteur d'intensité COMPACT (placé AVANT la scène) */}
          <div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Ex: Hier soir, Sarah a organisé un dîner. Elle a fait des sous-entendus sur Marc et Julie..."
              rows={6}
            ...
            <div className="flex justify-end mt-2">
              <span className={`text-xs font-bold ${scene.length < 50 ? "text-amber-600" : "text-emerald-600"}`}>
                {scene.length < 50 ? `${50 - scene.length} caractères min.` : "✅ Prêt"}
              </span>
            </div>
          </div>
